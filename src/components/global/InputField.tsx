...
import { SvgIcons } from '@/icons/SvgIconts';
  ...
  export default function InputField<T extends FieldValues>({
    containerClassName, type = 'text', fieldName,title, disabled = false, placeholder, error, register, className,showPasswordToggle,onpaswordToggle
  }:InputFieldType<T>) {
    const {
    ...
    return (
      <div className={cn('mb-5 w-full relative', containerClassName)}>
        <h3 className="my-1 pl-1 text-shade font-medium">{title}</h3>
